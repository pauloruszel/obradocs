  ...
  <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    <View style={styles.hero}>
      <View style={styles.heroBadge}>
        ...
        <Text style={styles.heroBadgeText}>PARA QUEM QUER CRESCER</Text>
      </View>
      <View style={styles.crownWrap}>
        <Crown size={30} color={colors.surface} />
      </View>
    ...
    <View style={styles.launchCard}>
      <View style={styles.launchHeader}>
        ...
        <Text style={styles.launchTitle}>Condição especial de lançamento</Text>
      </View>
      <Text style={styles.launchText}>
        Os primeiros clientes poderão receber uma condição exclusiva de fundador quando as assinaturas forem liberadas.
      </Text>
    </View>
    ...
    <View style={styles.table}>
      <View style={[styles.tableRow, styles.tableHeader]}>
        <Text style={[styles.cellFeature, styles.headerText]}>Recurso</Text>
        <Text style={[styles.cell, styles.headerText]}>Gratuito</Text>
        <Text style={[styles.cell, styles.proHeader]}>Profissional</Text>
      </View>
      {comparisons.map(([feature, free, pro], index) => (
        <View key={feature} style={[styles.tableRow, index < comparisons.length - 1 && styles.tableBorder]}>
          <Text style={styles.cellFeature}>{feature}</Text>
    ...
    <View style={styles.quoteCard}>
      <Text style={styles.quote}>“Seu escritório da obra na nuvem, disponível onde você estiver.”</Text>
    </View>
    ...
    <AppButton
      ...
      onPress={() => toastInfo(
        ...
        "Na próxima etapa você poderá entrar na lista de espera e receber a condição de lançamento.",
      )}
      style={styles.cta}
    />
...
const styles = StyleSheet.create({
  ...
  benefitTitle: { color: colors.text, fontSize: 16, fontWeight: "800" },
  benefitDescription: { color: colors.textMuted, lineHeight: 20, marginTop: 3 },
  launchCard: {
  ...
  launchHeader: { flexDirection: "row", alignItems: "center", gap: spacing.sm },
  launchTitle: { color: colors.text, fontSize: 17, fontWeight: "800" },
  launchText: { color: colors.textMuted, lineHeight: 21, marginTop: spacing.sm },
  table: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, overflow: "hidden" },
  tableRow: { flexDirection: "row", alignItems: "center", minHeight: 54, paddingHorizontal: spacing.sm },
  tableHeader: { backgroundColor: colors.primarySoft },
  tableBorder: { borderBottomWidth: 1, borderBottomColor: colors.border },
  cellFeature: { flex: 1.35, color: colors.text, fontSize: 13, fontWeight: "700" },
  cell: { flex: 1, color: colors.textMuted, fontSize: 12, textAlign: "center" },
  headerText: { color: colors.text, fontWeight: "800" },
  proHeader: { flex: 1.2, color: colors.primary, fontSize: 12, fontWeight: "800", textAlign: "center" },
  proCell: { flex: 1.2, flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 3 },
  proText: { color: colors.text, fontSize: 12, fontWeight: "700" },
  quoteCard: { marginTop: spacing.xl, borderLeftWidth: 4, borderLeftColor: colors.primary, padding: spacing.lg, backgroundColor: colors.surface },
  quote: { color: colors.text, fontSize: 17, lineHeight: 25, fontWeight: "700" },
